...
export const getCloudFunctionsCost = async (projectId, startOfMonthTs) => {
  ...
  netResult[0].forEach((entry) => {
    ...
    networkEgreessBytesV2 += entryBytes;
  });
  ...
  const paidNetworkEgressBytesV2 = Math.max(
    0,
    networkEgreessBytesV2 - FREE_NETWORK_EGRESS_BYTES_PER_MONTH,
  );
  ...
  log(
    `  Network Egress: ${paidNetworkEgressBytesV2.toFixed(2)} paid bytes (${paidEgressGb.toFixed(2)} GB) @ $${netPrice}/GB = $${netCost.toFixed(2)}. Used ${Math.min(FREE_NETWORK_EGRESS_BYTES_PER_MONTH, networkEgreessBytesV2.toFixed(2))}/${FREE_NETWORK_EGRESS_BYTES_PER_MONTH} free bytes`,
  );
  log(
